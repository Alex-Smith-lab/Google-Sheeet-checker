// Exact 43-column schema matching Header specifications
const masterHeaders = [
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Restore persistent workspace state and database on reload
  chrome.storage.local.get(['projectDatabase', 'appTheme', 'activeMainSheet', 'activeSubSheet'], (result) => {
    if (result.projectDatabase) {
      projectDatabase = result.projectDatabase;
      updateDropdownMenu();
      rebuildWorkbookTree();
    }
    if (result.appTheme === 'dark') {
      document.documentElement.setAttribute('data-theme', 'dark');
      document.getElementById('theme-toggle-btn').textContent = "☼ Light Mode";
    }

    if (result.activeMainSheet && result.activeSubSheet) {
      if (projectDatabase[result.activeMainSheet] && projectDatabase[result.activeMainSheet][result.activeSubSheet]) {
        switchViewContext(result.activeMainSheet, result.activeSubSheet);
      }
    }
...
// HTML Clipboard Capture for Strikethrough style detection
...
function parsePastedStreamForAssignees() {
  ...
  // Fallback scan if header not found in top row
  ...
  // Auto-detect Bus Route identifier if present
  let detectedRoute = "";
...
function selectAssigneeFilter(name) {
  ...
  // Auto-set folder name to selected user name
...
document.getElementById('theme-toggle-btn').addEventListener('click', () => {
  ...
  chrome.storage.local.set({ appTheme: targetTheme });
});
...
document.getElementById('btn-close-view').addEventListener('click', () => {
  activeMainSheet = "";
  activeSubSheet = "";
  chrome.storage.local.remove(['activeMainSheet', 'activeSubSheet']);
...
document.getElementById('process-entry-btn').addEventListener('click', () => {
  ...
  setTimeout(() => {
    ...
    lines.forEach((line, index) => {
      ...
      // Filter by selected assignee if chosen
      ...
      // Ignore standard duplicated header rows
    ...
    chrome.storage.local.set({ projectDatabase: projectDatabase }, () => {
      overlay.classList.add('hidden');
...
function switchViewContext(mKey, sKey) {
  ...
  chrome.storage.local.set({ activeMainSheet: mKey, activeSubSheet: sKey });
...
document.getElementById('btn-create-gsheet').addEventListener('click', () => {
  ...
  navigator.clipboard.writeText(fullDataClipboardString).then(() => {
    alert("Formatted data loaded to clipboard. Opening a new Google Sheet... Press Ctrl+V (or Cmd+V) to paste.");
...
document.getElementById('clear-db-btn').addEventListener('click', () => {
  if (confirm("Permanently wipe local workspace database memory?")) {
    chrome.storage.local.remove(['projectDatabase', 'activeMainSheet', 'activeSubSheet'], () => {
      projectDatabase = {};
      document.getElementById('btn-close-view').click();
      updateDropdownMenu();
      rebuildWorkbookTree();
